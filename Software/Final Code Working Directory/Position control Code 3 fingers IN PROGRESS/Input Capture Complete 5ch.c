...
unsigned int NORMALIZATION_CONSTANT = 1;                                        // "Self-explanatory" - for encoder position values. Was 4
...
struct finger {
        ...
        unsigned long FULLY_CONTRACTED;                                         // Upper bound on value for position of finger
...
void init_GPIO() {
 ...
 void init_pointer_PWM( ) {
     ...
     PWM_TIM1_Set_Duty(70*(pwm_period/100), _PWM_NON_INVERTED, POINTER_PWM);    // Set 70% duty on Timer 1, channel 1
 ...
 void init_middle_PWM( ) {		
	...
	pwm_period = PWM_TIM1_Init(PWM_FREQ_HZ);                                // Set PWM base frequency to 100 Hz
	PWM_TIM1_Set_Duty(70*(pwm_period/100), _PWM_NON_INVERTED, MIDDLE_PWM);  // PWM duty cycle to 0 on Timer 1, channel 2
 ...
 void init_ring_PWM( ) {
	...
	pwm_period = PWM_TIM1_Init(PWM_FREQ_HZ);                               	// Set PWM base frequency to 100 Hz
	PWM_TIM1_Set_Duty(70*(pwm_period/100), _PWM_NON_INVERTED, RING_PWM);          		// PWM duty cycle to 0 on Timer 1, channel 3
 ...
 void init_pinky_PWM( ) {
	...
	pwm_period = PWM_TIM1_Init(PWM_FREQ_HZ);                                // Set PWM base frequency to 100 Hz
	PWM_TIM1_Set_Duty(70*(pwm_period/100), _PWM_NON_INVERTED, PINKY_PWM);          		// PWM duty cycle to 0 on Timer 1, channel 4
 ...
 void init_thumb_PWM( ) {
	...
	PWM_TIM4_Set_Duty(70*(pwm_period/100), _PWM_NON_INVERTED, THUMB_PWM);          		// PWM duty cycle to 0 on Timer 4, channel 1
 ...
 void init_finger(struct finger *fngr)
 {
      ...
      MIDDLE_DIRECTION = CONTRACT;
      RING_DIRECTION = CONTRACT;
      PINKY_DIRECTION = CONTRACT;
      THUMB_DIRECTION = CONTRACT;
...
void calc_finger_state( struct finger *fngr) {
   ...
   // fngr->position_actual = (long) fngr->position_actual / 4.0;
    // NEW
    if(fngr->position_actual >= fngr->FULLY_CONTRACTED) {  // don't run too far!
